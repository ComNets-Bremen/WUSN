...
class RFM69(object):
    def __init__(self, freqBand, nodeID, networkID, isRFM69HW = False, intPin = 'P9', rstPin = 'P22', spiBus = 0, spiDevice = 0, wakeup = False):
        if wakeup == False:
            ...
            self.spi = SPI(0, mode=SPI.MASTER, baudrate=400000, polarity=0, phase=0, firstbit = SPI.MSB)
...
#    replies usually take only 5-8ms at 50kbps@915Mhz
    ...
    def sendFrame(self, toAddress, buff, requestACK, sendACK):
        ...
        if isinstance(buff, str): #original basestr
            self.spiWriteMultipleBytes([REG_FIFO | 0x80, len(buff) + 3, toAddress, self.address, ack] + [int(ord(i)) for i in list(buff)])
        else:
            self.spiWriteMultipleBytes([REG_FIFO | 0x80, len(buff) + 3, toAddress, self.address, ack] + buff)
    ...
    def interruptHandler(self, pin):
        ...
        if self.mode == RF69_MODE_RX and self.readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY:
            self.setMode(RF69_MODE_STANDBY)
            self.PAYLOADLEN, self.TARGETID, self.SENDERID, CTLbyte = self.readFifo([REG_FIFO & 0x7f,0,0,0,0])[1:]
            ...
            self.DATA = self.readFifo([REG_FIFO & 0x7f] + [0 for i in range(0, self.DATALEN)])[1:]
