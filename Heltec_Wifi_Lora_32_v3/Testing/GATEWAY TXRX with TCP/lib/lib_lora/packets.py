...
class BasePacket:
    ...
    def create_packet(self):
        ...
        p = struct.pack("!BHIBI", self.version, self.seq, self.sender, self.config, self.retry_count)
    ...
    def parse_packet(self, packet):
        ...
        self.version, self.seq, self.sender, self.config, self.retry_count = struct.unpack("!BHIBI", p[:12])
        p = p[12:]
        if not self.get_broadcast():
