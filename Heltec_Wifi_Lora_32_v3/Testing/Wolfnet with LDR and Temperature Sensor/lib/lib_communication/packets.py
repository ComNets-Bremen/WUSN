...
class BasePacket:
    ...
    TYPE_CONFIG     = 0x3
    TYPE_UNIVERSAL    = 0x10
    TYPE_LDR_READING = 0x11
    TYPE_TEMP_READING = 0x12
...
# Sensor Node Reading packet types
class LDRPacket(UniversalPacket):
    def __init__(self, sender=None):
        super().__init__(sender)
        self.param_type = "!f"
        self.set_type(self.TYPE_LDR_READING)

    def get_params(self):
        if len(self.data) == struct.calcsize(self.param_type):
            data = struct.unpack(self.param_type, self.data)
            return float(str(data)[1:-2])
        else:
            raise ValueError("Wrong Packet format")
        
class TempPacket(UniversalPacket):
    def __init__(self, sender=None):
        super().__init__(sender)
        self.param_type = "!f"
        self.set_type(self.TYPE_TEMP_READING)
