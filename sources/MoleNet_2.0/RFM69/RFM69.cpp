...
volatile uint16_t RFM69::RTT;
volatile uint8_t RFM69::RETERIES;
...
bool RFM69::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  const uint8_t CONFIG[][2] =
  {
    ...
    /* 0x03 */ { REG_BITRATEMSB, RF_BITRATEMSB_55555}, //RF_BITRATEMSB_55555} RF_BITRATEMSB_1200, // default: 4.8 KBPS
    /* 0x04 */ { REG_BITRATELSB, RF_BITRATELSB_55555}, //RF_BITRATELSB_55555},RF_BITRATELSB_1200 
...
bool RFM69::sendWithRetry(uint8_t toAddress, const void* buffer, uint8_t bufferSize, uint8_t retries, uint16_t retryWaitTime) {
  ...
  for (; i <= retries; i++)
  {
    ...
    while (millis() - sentTime < retryWaitTime)
    {
      ...
      {  //::TODO may RTT should be zero in case tx fails
        RTT = millis() - sentTime;
        RETERIES = i;
        Serial.print(" ~RTT("); Serial.print(RTT); Serial.print(" ms)");
        Serial.print(" #Retries("); Serial.print(i); Serial.print(")");
        return true;
      }
    }
    //Serial.print(" RETRY#"); Serial.println(i + 1);
  }
    Serial.print(" #Retries("); Serial.print(i--); Serial.print(")");   // decrement to compensate last for loop iteration
    return false;
}
