...
void mainLoop()
{
    ...
    if( (millis() - currentTime) > (node.config[StartupConfig::pos_DELAY_BEFORE_SEND]*1000 ) && ifDelayBeforeSending ) {
        ...
        node.exciteSensor();                  // Excite Sensor, Activate DDAC

//        node.send_typ_RX_TX_INFORMATION(         // Send Information Packet
//            node.config[StartupConfig::pos_GATEWAYID],
//            node.config[StartupConfig::pos_SINKID] );

//        node.send_typ_MOISTURE_TEMP_SENSOR(      // Send Moisture Packet
//            node.config[StartupConfig::pos_GATEWAYID],
//            node.config[StartupConfig::pos_SINKID] );           
    }

    // DDAC: Decagon5TM Data Arrival Condition
    if( node.newDecagon5tmDataAvailable() ) {  
        node.send_typ_DATA_PLUS_INFO(             // Send sensor information to gateway
          node.config[StartupConfig::pos_GATEWAYID], 
          node.config[StartupConfig::pos_SINKID] );  

//        node.send_typ_DECAGON5TM_SENSOR(        // Send sensor information to gateway
//          node.config[StartupConfig::pos_GATEWAYID],
//          node.config[StartupConfig::pos_SINKID]);            
    }        
