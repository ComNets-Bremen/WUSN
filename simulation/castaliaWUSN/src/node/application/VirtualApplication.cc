...
void VirtualApplication::notifyWirelessChannelfromApp(double VWC)
{
    channelUpdateMsg =
    new WirelessChannelUpdateMessage ("channel update message", WC_UPDATE_VWC);

    channelUpdateMsg->setPLd0_VWC(VWC);
    channelUpdateMsg->setNodeID(getParentModule()->getIndex());

   // EV << "VWC value updated" << "\n";

    sendDirect(channelUpdateMsg, wchannel, "fromAppModule");
}
